...
3. Preflight worker bundles with `validate-execution-loads.mjs`, then invoke `canonical-worker`; supply task, allowlist, stop conditions, route path and load-bundle path.
...
6. Preflight `review-route.json` + `review-loads.json`, then invoke `canonical-reviewer` in a different session and require `REVIEW-E` targeting the worker receipt.
7. Persist only the receipt/evidence envelope, never invented execution data.
8. Run `node .agents/scripts/validate-execution-evidence.mjs --evidence <dir>/execution-evidence.json --evidence-dir <dir> --output <dir>/validation-evidence.json`.
