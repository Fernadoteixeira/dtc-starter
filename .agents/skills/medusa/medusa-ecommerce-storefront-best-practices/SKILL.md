---
name: medusa-ecommerce-storefront-best-practices
...
This skill was migrated from the Claude plugin `ecommerce-storefront` (origin: `medusajs/medusa-agent-skills`, commit `c584f79`) to the VS Code Copilot skill format. The canonical name is `medusa-ecommerce-storefront-best-practices`.
...
- **Project-specific conventions (from AGENTS.md):** This workspace uses **pnpm** (pinned at `pnpm@10.11.1`). The storefront is **Next.js 15.5 with Turbopack** on port 8000. It uses **Tailwind CSS v3** (not v4) — v4-only syntax breaks the build. **Framer Motion and shadcn/ui are NOT installed** — use Radix UI primitives and Headless UI directly. When applying this skill's patterns to the `apps/storefront/` codebase, always respect these constraints.
- **Medusa MCP server:** Per AGENTS.md: "a `medusa` MCP server exposing the official docs. Prefer it over web search or memory for any Medusa API, config, or upgrade question." If it is not connected, suggest: `claude mcp add --transport http medusa https://docs.medusajs.com/mcp` or the VS Code Copilot equivalent.
- **Canonical name:** The `name` field is `medusa-ecommerce-storefront-best-practices` (matching the directory name), not `storefront-best-practices` from the original. When referencing this skill, use the canonical name.

**Auxiliary files preserved exactly (27 reference files):**

| Path | Content |
|------|---------|
| `reference/connecting-to-backend.md` | Framework detection, API setup, backend integration |
| `reference/medusa.md` | Medusa SDK integration, pricing, regions, TypeScript types |
| `reference/design.md` | User preferences, brand identity, design systems |
| `reference/seo.md` | Meta tags, structured data, Core Web Vitals |
...
| `reference/components/breadcrumbs.md` | Category hierarchy, structured data markup |
| `reference/components/cart-popup.md` | Add-to-cart feedback, mini cart display |
| `reference/components/country-selector.md` | Country/region selection, currency, pricing |
| `reference/components/footer.md` | Content organization, navigation, social media |
| `reference/components/hero.md` | Hero layouts, CTA placement, image optimization |
| `reference/components/megamenu.md` | Category organization, featured products |
| `reference/components/navbar.md` | Desktop/mobile navigation, logo, menu, cart icon |
| `reference/components/popups.md` | Newsletter signup, discount popups, exit-intent |
| `reference/components/product-card.md` | Product images, pricing, add to cart, badges |
| `reference/components/product-reviews.md` | Review display, rating aggregation, submission |
| `reference/components/product-slider.md` | Carousel implementation, mobile swipe, accessibility |
