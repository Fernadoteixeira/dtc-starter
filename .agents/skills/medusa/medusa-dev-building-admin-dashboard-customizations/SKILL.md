---
name: medusa-dev-building-admin-dashboard-customizations
...
- **Creating widgets?** → MUST load [references/data-loading.md](references/data-loading.md) first
- **Building forms/modals?** → MUST load [references/forms.md](references/forms.md) first
- **Displaying data in tables/lists?** → MUST load [references/display-patterns.md](references/display-patterns.md) first
- **Selecting from large datasets?** → MUST load [references/table-selection.md](references/table-selection.md) first
- **Adding navigation?** → MUST load [references/navigation.md](references/navigation.md) first
- **Styling components?** → MUST load [references/typography.md](references/typography.md) first
...
- [references/data-loading.md](references/data-loading.md) - useQuery/useMutation patterns, cache invalidation
- [references/forms.md](references/forms.md) - FocusModal/Drawer patterns, validation
- [references/table-selection.md](references/table-selection.md) - Complete DataTable selection pattern
- [references/display-patterns.md](references/display-patterns.md) - Lists, tables, cards for entities
- [references/typography.md](references/typography.md) - Text component patterns
- [references/navigation.md](references/navigation.md) - Link, useNavigate, useParams patterns
...
This skill was migrated from the Claude plugin `medusa-dev` (origin: `medusajs/medusa-agent-skills`, commit `c584f79`) to the VS Code Copilot skill format. The canonical name is `medusa-dev-building-admin-dashboard-customizations`.
...
- **Destructive operations require human confirmation:** Operations such as database migrations, package installations, or any destructive commands must be explicitly confirmed by the user before execution. Never run destructive commands automatically.
- **Canonical name:** The `name` field is `medusa-dev-building-admin-dashboard-customizations` (matching the directory name), not `building-admin-dashboard-customizations` from the original. When referencing this skill, use the canonical name.
