---
name: medusa-cloud-deployments
description: Execute mcloud deployments commands to list deployments, retrieve deployment details, and fetch build logs. Use when listing deployments, checking deployment status, or reading build output for debugging build failures. All deployment commands are read-only and safe to run without confirmation.
---
...
This skill was migrated from the Claude plugin `medusa-cloud` (origin: `medusajs/medusa-agent-skills`, commit `c584f79`) to the VS Code Copilot skill format. The canonical name is `medusa-cloud-deployments`.
...
- **No `allowed-tools` restriction:** The Claude original frontmatter included `allowed-tools: Bash(mcloud deployments*), Bash(jq*)`. Copilot does not support this field. Copilot uses normal terminal approval workflows — the user is prompted before terminal commands execute. The intended scope is documented here: this skill should only run `mcloud deployments` commands (`list`, `get`, `build-logs`) and `jq` for output parsing.
- **Cloud operations can be irreversible:** While `mcloud deployments` commands are read-only (list, get, build-logs) and safe to run without confirmation, the Cloud environment they inspect is live and shared. Always verify the organization and project context (`mcloud whoami --json`) before running commands to avoid reading data from the wrong project. Never attempt to derive or guess deployment IDs — always obtain them from `deployments list` first.
- **Canonical name:** The `name` field is `medusa-cloud-deployments` (matching the directory name), not `mcloud-deployments` from the original. When referencing this skill, use the canonical name.
