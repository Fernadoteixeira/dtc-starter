---
name: medusa-learning-medusa
description: Load automatically when user asks to learn Medusa development (e.g., "teach me how to build with medusa", "guide me through medusa", "I want to learn medusa"). Interactive guided tutorial where the agent acts as a coding bootcamp instructor, teaching step-by-step with checkpoints and verification.
---
...
This is NOT a passive reference skill. This is an **INTERACTIVE TUTORING SESSION** where you (the agent) guide the user through building a brands feature in Medusa, teaching architecture concepts along the way.
...
As the agent, you are a patient, thorough coding bootcamp instructor teaching Medusa development. Your goals:
...
This skill was migrated from the Claude plugin `learn-medusa` (origin: `medusajs/medusa-agent-skills`, commit `c584f79`) to the VS Code Copilot skill format. The canonical name is `medusa-learning-medusa`.
...
- **No `allowed-tools` restriction:** The Claude original did not include `allowed-tools` in frontmatter. Copilot does not support this field regardless. Copilot uses normal terminal approval workflows — the user is prompted before terminal commands execute.
- **Agent identity:** The original refers to "Claude" as the instructor. In this canonical version, references to "Claude" have been replaced with "the agent" to be vendor-neutral. The tutoring protocol, lesson structure, checkpoints, architecture deep dives, troubleshooting guides, and all auxiliary files are preserved exactly as in the original.
- **MCP server:** The original references the MedusaDocs MCP server (`ask_medusa_question` tool). If the Medusa MCP server is connected in VS Code Copilot, use it equivalently. If it is not connected, suggest the setup command: `claude mcp add --transport http medusa https://docs.medusajs.com/mcp` or the VS Code Copilot equivalent. Per AGENTS.md: "a `medusa` MCP server exposing the official docs. Prefer it over web search or memory for any Medusa API, config, or upgrade question."
- **Package manager convention:** This workspace uses **pnpm** (pinned at `pnpm@10.11.1`). When the tutorial references `npm run build` or similar, substitute `pnpm run build` or `pnpm exec <command>` as appropriate.
- **Canonical name:** The `name` field is `medusa-learning-medusa` (matching the directory name), not `learning-medusa` from the original. When referencing this skill, use the canonical name.

**Auxiliary files preserved exactly:**

| Path | Content |
|------|---------|
| `architecture/admin-integration.md` | Admin UI integration patterns |
| `architecture/module-isolation.md` | Module isolation principles |
| `architecture/module-workflow-route.md` | Module → Workflow → Route pattern |
| `architecture/workflow-orchestration.md` | Workflow orchestration and hooks |
| `checkpoints/checkpoint-api-route.md` | API route checkpoint |
| `checkpoints/checkpoint-module-links.md` | Module links checkpoint |
| `checkpoints/checkpoint-module.md` | Module creation checkpoint |
| `checkpoints/checkpoint-querying.md` | Querying checkpoint |
| `checkpoints/checkpoint-ui-route.md` | UI route checkpoint |
| `checkpoints/checkpoint-widget.md` | Widget checkpoint |
| `checkpoints/checkpoint-workflow-hooks.md` | Workflow hooks checkpoint |
| `checkpoints/checkpoint-workflow.md` | Workflow checkpoint |
| `lessons/lesson-1-custom-features.md` | Lesson 1: Custom features |
| `lessons/lesson-2-extend-medusa.md` | Lesson 2: Extending Medusa |
| `lessons/lesson-3-admin-dashboard.md` | Lesson 3: Admin dashboard |
| `troubleshooting/common-errors.md` | Common errors guide |
| `troubleshooting/verification-guide.md` | Verification guide |
