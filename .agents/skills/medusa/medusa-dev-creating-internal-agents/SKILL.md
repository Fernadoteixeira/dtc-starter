---
name: medusa-dev-creating-internal-agents
...
This skill was migrated from the Claude plugin `medusa-dev` (origin: `medusajs/medusa-agent-skills`, commit `c584f79`) to the VS Code Copilot skill format. The canonical name is `medusa-dev-creating-internal-agents`.
...
- **Name mismatch resolved:** The original Claude frontmatter `name` was `creating-agents-in-medusa`, but the canonical directory (and the `name` field in this SKILL.md) is `medusa-dev-creating-internal-agents`. This aligns with the VS Code Copilot discovery rule that `name` must match the parent directory name.
...
- **Destructive operations require human confirmation:** Operations such as `db:migrate`, `db:generate`, package installations, or any database mutations must be explicitly confirmed by the user before execution. Never run destructive commands automatically.
- **Package manager convention:** This workspace uses `pnpm`. When running migration commands referenced in this skill, use `pnpm exec medusa db:generate` and `pnpm exec medusa db:migrate` instead of `npx medusa db:generate` / `npx medusa db:migrate`. Similarly, use `pnpm exec medusa develop` instead of `npx medusa develop`.

**Auxiliary reference files copied alongside this skill:**

- [reference/data-models.md](reference/data-models.md) - model.define(), agent_type discriminator, relationships, migrations
- [reference/service.md](reference/service.md) - MedusaService extension, Anthropic init, stream(), module index, config registration
- [reference/agent-setup.md](reference/agent-setup.md) - streamText(), MedusaExec tool wiring, system prompt, context passing
- [reference/api-route.md](reference/api-route.md) - POST route, session lifecycle, message persistence, streaming headers
- [reference/streaming.md](reference/streaming.md) - NDJSON emission, fullStream iteration, chunk types, client-side parsing
- [reference/admin-extension.md](reference/admin-extension.md) - React chat UI, streaming fetch, message rendering, tool call display, session sidebar
- [reference/medusa-exec.md](reference/medusa-exec.md) - Executor setup, MedusaExec tool, query.graph() patterns, error codes
