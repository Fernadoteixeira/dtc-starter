...
maturity: "canonical"
version: "1.0.0"
tags: ["applications", "client", "data", "design", "flow", "frontend", "shared", "state"]
---
...
Produce a defensible, executable and measurable approach to **shared state design** within the broader capability of **state and data flow**, independent of industry, technology stack, programming language or delivery method.

## Capability context

Controls local, shared and server-synchronized state.

This skill owns the decision and execution contract for **shared state design**. It may audit an existing state, design a target state, guide implementation, verify results, govern ongoing operation or support controlled evolution.

## Invoke when

- The product lifecycle requires a material decision about **shared state design**.
...
- Shared State Design decision brief
...
1. Frame the exact decision or outcome governed by shared state design.
...
```text
Act as the accountable owner for Shared State Design.
