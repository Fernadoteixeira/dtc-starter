---
id: "PLC-103"
name: "local-state-design"
title: "Local State Design"
...
maturity: "canonical"
version: "1.0.0"
tags: ["applications", "client", "data", "design", "flow", "frontend", "local", "state"]
---
...
Produce a defensible, executable and measurable approach to **local state design** within the broader capability of **state and data flow**, independent of industry, technology stack, programming language or delivery method.

## Capability context

Controls local, shared and server-synchronized state.

This skill owns the decision and execution contract for **local state design**. It may audit an existing state, design a target state, guide implementation, verify results, govern ongoing operation or support controlled evolution.

## Invoke when

- The product lifecycle requires a material decision about **local state design**.
...
- Local State Design decision brief
...
1. Frame the exact decision or outcome governed by local state design.
...
```text
Act as the accountable owner for Local State Design.
