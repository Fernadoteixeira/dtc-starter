...
maturity: "canonical"
version: "1.0.0"
tags: ["change", "critical", "delivery", "management", "path", "planning", "release"]
---
...
Produce a defensible, executable and measurable approach to **critical path management** within the broader capability of **delivery planning**, independent of industry, technology stack, programming language or delivery method.

## Capability context

Decomposes and sequences work against constraints.

This skill owns the decision and execution contract for **critical path management**. It may audit an existing state, design a target state, guide implementation, verify results, govern ongoing operation or support controlled evolution.

## Invoke when

- The product lifecycle requires a material decision about **critical path management**.
...
- Critical Path Management decision brief
...
1. Frame the exact decision or outcome governed by critical path management.
...
```yaml
skill_id: PLC-260
skill: critical-path-management
...
```text
Act as the accountable owner for Critical Path Management.
