 ...
 *
 * @param {object} params
 * @param {string} params.method - "GET" (AgentCard) or "POST" (SendMessage / JSON-RPC)
...
export async function handleGatewayRequest({ method, path: reqPath, headers = {}, body = {} }) {
  ...
  // -------------------------------------------------------------------------
  // 4. Dispatch JSON-RPC Method or Direct SendMessage
  // -------------------------------------------------------------------------
  ...
  if (rpcMethod !== "SendMessage" && rpcMethod !== "a2a.sendMessage") {
    return {
      ...
      body: {
        jsonrpc: "2.0",
        id: rpcId,
        error: { code: -32601, message: `Method not found: ${rpcMethod}. Supported: SendMessage` },
      },
    }
  }
  ...
  // -------------------------------------------------------------------------
  // 7. Sanitization & Artifact Separation
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // 8. Official A2A Task Response Construction
  // -------------------------------------------------------------------------
  const responseTask = {
    id: capsule.task_id,
    status: {
