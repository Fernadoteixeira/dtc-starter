---
name: "react-type-contracts"
title: "React Type Contracts"
capability_id: "SK-204"
...
# React Type Contracts

## Purpose

Design, implement, review and troubleshoot **React Type Contracts** using `@types/react` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **React Type Contracts**.
- A design or implementation decision depends on `@types/react`.
...
- The task only asks whether `@types/react` is installed.
...
1. Clarify the desired outcome for react type contracts and identify the user-visible or operational result.
2. Inspect the relevant `@types/react` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides react type contracts, not only a wrapper or installed dependency.
