---
name: "react-dom-type-contracts"
title: "React DOM Type Contracts"
capability_id: "SK-205"
...
# React DOM Type Contracts

## Purpose

Design, implement, review and troubleshoot **React DOM Type Contracts** using `@types/react-dom` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **React DOM Type Contracts**.
- A design or implementation decision depends on `@types/react-dom`.
...
- The task only asks whether `@types/react-dom` is installed.
...
1. Clarify the desired outcome for react dom type contracts and identify the user-visible or operational result.
2. Inspect the relevant `@types/react-dom` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides react dom type contracts, not only a wrapper or installed dependency.
