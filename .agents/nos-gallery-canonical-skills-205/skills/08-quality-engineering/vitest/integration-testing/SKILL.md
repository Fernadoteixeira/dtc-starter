...
Design, implement, review and troubleshoot **Integration Testing** using `vitest` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Integration Testing**.
- A design or implementation decision depends on `vitest`.
...
- The task only asks whether `vitest` is installed.
...
1. Clarify the desired outcome for integration testing and identify the user-visible or operational result.
2. Inspect the relevant `vitest` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides integration testing, not only a wrapper or installed dependency.
