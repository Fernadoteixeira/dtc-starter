---
name: "node-type-contracts"
title: "Node.js Type Contracts"
capability_id: "SK-202"
...
# Node.js Type Contracts

## Purpose

Design, implement, review and troubleshoot **Node.js Type Contracts** using `@types/node` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Node.js Type Contracts**.
- A design or implementation decision depends on `@types/node`.
...
- The task only asks whether `@types/node` is installed.
...
1. Clarify the desired outcome for node.js type contracts and identify the user-visible or operational result.
2. Inspect the relevant `@types/node` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides node.js type contracts, not only a wrapper or installed dependency.
