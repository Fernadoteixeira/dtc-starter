...
Design, implement, review and troubleshoot **Coverage Collection** using `@vitest/coverage-v8` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Coverage Collection**.
- A design or implementation decision depends on `@vitest/coverage-v8`.
...
- The task only asks whether `@vitest/coverage-v8` is installed.
...
1. Clarify the desired outcome for coverage collection and identify the user-visible or operational result.
2. Inspect the relevant `@vitest/coverage-v8` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides coverage collection, not only a wrapper or installed dependency.
