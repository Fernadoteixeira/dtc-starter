---
name: "postgres-type-contracts"
title: "PostgreSQL Type Contracts"
capability_id: "SK-203"
...
# PostgreSQL Type Contracts

## Purpose

Design, implement, review and troubleshoot **PostgreSQL Type Contracts** using `@types/pg` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **PostgreSQL Type Contracts**.
- A design or implementation decision depends on `@types/pg`.
...
- The task only asks whether `@types/pg` is installed.
...
1. Clarify the desired outcome for postgresql type contracts and identify the user-visible or operational result.
2. Inspect the relevant `@types/pg` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides postgresql type contracts, not only a wrapper or installed dependency.
