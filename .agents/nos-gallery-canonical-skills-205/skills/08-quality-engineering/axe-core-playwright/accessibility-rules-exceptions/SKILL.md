---
name: "accessibility-rules-exceptions"
title: "Accessibility Rules and Exceptions"
capability_id: "SK-175"
domain: "08-quality-engineering"
...
# Accessibility Rules and Exceptions

## Purpose

Design, implement, review and troubleshoot **Accessibility Rules and Exceptions** using `@axe-core/playwright` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Accessibility Rules and Exceptions**.
- A design or implementation decision depends on `@axe-core/playwright`.
...
- The task only asks whether `@axe-core/playwright` is installed.
...
- system under test
- acceptance criteria
- CI and environment constraints
...
1. Clarify the desired outcome for accessibility rules and exceptions and identify the user-visible or operational result.
2. Inspect the relevant `@axe-core/playwright` contracts, runtime boundaries and surrounding architecture.
...
- Tests must verify user-visible outcomes and executable contracts.
- Avoid brittle selectors, timing sleeps and snapshots without intent.
- Make quality gates deterministic, reproducible and CI-friendly.

## Acceptance criteria

- [ ] The solution demonstrably provides accessibility rules and exceptions, not only a wrapper or installed dependency.
