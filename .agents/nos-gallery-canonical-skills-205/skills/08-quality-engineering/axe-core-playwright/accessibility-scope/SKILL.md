...
Design, implement, review and troubleshoot **Accessibility Audit Scope** using `@axe-core/playwright` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Accessibility Audit Scope**.
...
1. Clarify the desired outcome for accessibility audit scope and identify the user-visible or operational result.
...
- [ ] The solution demonstrably provides accessibility audit scope, not only a wrapper or installed dependency.
