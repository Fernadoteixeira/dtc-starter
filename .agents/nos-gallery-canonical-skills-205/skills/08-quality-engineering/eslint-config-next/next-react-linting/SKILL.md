---
name: "next-react-linting"
title: "Next.js and React Linting"
capability_id: "SK-198"
...
# Next.js and React Linting

## Purpose

Design, implement, review and troubleshoot **Next.js and React Linting** using `eslint-config-next` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Next.js and React Linting**.
- A design or implementation decision depends on `eslint-config-next`.
...
- The task only asks whether `eslint-config-next` is installed.
...
1. Clarify the desired outcome for next.js and react linting and identify the user-visible or operational result.
2. Inspect the relevant `eslint-config-next` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides next.js and react linting, not only a wrapper or installed dependency.
