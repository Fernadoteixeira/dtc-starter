---
name: "fixtures-isolation"
title: "Fixtures and Test Isolation"
capability_id: "SK-167"
...
Design, implement, review and troubleshoot **Fixtures and Test Isolation** using `@playwright/test` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Fixtures and Test Isolation**.
- A design or implementation decision depends on `@playwright/test`.
...
- The task only asks whether `@playwright/test` is installed.
...
1. Clarify the desired outcome for fixtures and test isolation and identify the user-visible or operational result.
2. Inspect the relevant `@playwright/test` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides fixtures and test isolation, not only a wrapper or installed dependency.
