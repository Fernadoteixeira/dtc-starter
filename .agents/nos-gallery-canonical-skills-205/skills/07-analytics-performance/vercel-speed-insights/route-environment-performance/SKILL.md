---
name: "route-environment-performance"
title: "Route and Environment Performance"
capability_id: "SK-164"
domain: "07-analytics-performance"
...
# Route and Environment Performance

## Purpose

Design, implement, review and troubleshoot **Route and Environment Performance** using `@vercel/speed-insights` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Route and Environment Performance**.
- A design or implementation decision depends on `@vercel/speed-insights`.
...
- The task only asks whether `@vercel/speed-insights` is installed.
...
1. Clarify the desired outcome for route and environment performance and identify the user-visible or operational result.
2. Inspect the relevant `@vercel/speed-insights` contracts, runtime boundaries and surrounding architecture.
...
- Do not emit PII, secrets or sensitive business payloads.
- Define an owner and decision for every collected signal.
- Measure production percentiles rather than relying only on local averages.

## Acceptance criteria

- [ ] The solution demonstrably provides route and environment performance, not only a wrapper or installed dependency.
