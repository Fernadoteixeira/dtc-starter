---
name: "toast-actions-lifecycle"
title: "Toast Actions and Lifecycle"
capability_id: "SK-154"
...
# Toast Actions and Lifecycle

## Purpose

Design, implement, review and troubleshoot **Toast Actions and Lifecycle** using `sonner` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Toast Actions and Lifecycle**.
- A design or implementation decision depends on `sonner`.
...
- The task only asks whether `sonner` is installed.
...
1. Clarify the desired outcome for toast actions and lifecycle and identify the user-visible or operational result.
2. Inspect the relevant `sonner` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides toast actions and lifecycle, not only a wrapper or installed dependency.
