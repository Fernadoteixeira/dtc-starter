---
name: "command-keyboard-navigation"
title: "Command Keyboard Navigation"
capability_id: "SK-137"
...
Design, implement, review and troubleshoot **Command Keyboard Navigation** using `cmdk` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Command Keyboard Navigation**.
- A design or implementation decision depends on `cmdk`.
...
- The task only asks whether `cmdk` is installed.
...
1. Clarify the desired outcome for command keyboard navigation and identify the user-visible or operational result.
2. Inspect the relevant `cmdk` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides command keyboard navigation, not only a wrapper or installed dependency.
