...
title: "Toggle Group Selection"
capability_id: "SK-097"
domain: "04-radix-ui"
...
Design, implement, review and troubleshoot **Toggle Group Selection** using `@radix-ui/react-toggle-group` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Toggle Group Selection**.
- A design or implementation decision depends on `@radix-ui/react-toggle-group`.
...
- The task only asks whether `@radix-ui/react-toggle-group` is installed.
...
1. Clarify the desired outcome for toggle group selection and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-toggle-group` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides toggle group selection, not only a wrapper or installed dependency.
