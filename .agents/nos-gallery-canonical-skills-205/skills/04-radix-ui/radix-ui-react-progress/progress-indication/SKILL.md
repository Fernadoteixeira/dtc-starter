...
Design, implement, review and troubleshoot **Progress Indication** using `@radix-ui/react-progress` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Progress Indication**.
- A design or implementation decision depends on `@radix-ui/react-progress`.
...
- The task only asks whether `@radix-ui/react-progress` is installed.
...
1. Clarify the desired outcome for progress indication and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-progress` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides progress indication, not only a wrapper or installed dependency.
