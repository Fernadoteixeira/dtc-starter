---
name: "custom-scroll-area"
title: "Custom Scroll Area"
capability_id: "SK-085"
domain: "04-radix-ui"
...
Design, implement, review and troubleshoot **Custom Scroll Area** using `@radix-ui/react-scroll-area` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Custom Scroll Area**.
- A design or implementation decision depends on `@radix-ui/react-scroll-area`.
...
- The task only asks whether `@radix-ui/react-scroll-area` is installed.
...
1. Clarify the desired outcome for custom scroll area and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-scroll-area` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides custom scroll area, not only a wrapper or installed dependency.
