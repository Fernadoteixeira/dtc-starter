...
Design, implement, review and troubleshoot **Collapsible Region** using `@radix-ui/react-collapsible` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Collapsible Region**.
- A design or implementation decision depends on `@radix-ui/react-collapsible`.
...
- The task only asks whether `@radix-ui/react-collapsible` is installed.
...
1. Clarify the desired outcome for collapsible region and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-collapsible` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides collapsible region, not only a wrapper or installed dependency.
