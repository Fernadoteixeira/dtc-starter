---
name: "select-options-navigation"
title: "Select Options Navigation"
capability_id: "SK-087"
domain: "04-radix-ui"
...
# Select Options Navigation

## Purpose

Design, implement, review and troubleshoot **Select Options Navigation** using `@radix-ui/react-select` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Select Options Navigation**.
- A design or implementation decision depends on `@radix-ui/react-select`.
...
- The task only asks whether `@radix-ui/react-select` is installed.
...
1. Clarify the desired outcome for select options navigation and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-select` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides select options navigation, not only a wrapper or installed dependency.
