---
name: "control-labeling"
title: "Control Labeling"
capability_id: "SK-076"
domain: "04-radix-ui"
...
Design, implement, review and troubleshoot **Control Labeling** using `@radix-ui/react-label` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Control Labeling**.
- A design or implementation decision depends on `@radix-ui/react-label`.
...
- The task only asks whether `@radix-ui/react-label` is installed.
...
1. Clarify the desired outcome for control labeling and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-label` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides control labeling, not only a wrapper or installed dependency.
