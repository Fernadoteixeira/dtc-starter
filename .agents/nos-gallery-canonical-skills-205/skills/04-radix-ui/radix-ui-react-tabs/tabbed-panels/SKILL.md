...
Design, implement, review and troubleshoot **Tabbed Panels** using `@radix-ui/react-tabs` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Tabbed Panels**.
- A design or implementation decision depends on `@radix-ui/react-tabs`.
...
- The task only asks whether `@radix-ui/react-tabs` is installed.
...
1. Clarify the desired outcome for tabbed panels and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-tabs` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides tabbed panels, not only a wrapper or installed dependency.
