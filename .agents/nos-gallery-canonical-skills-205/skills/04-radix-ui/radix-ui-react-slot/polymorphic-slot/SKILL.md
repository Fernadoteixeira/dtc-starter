...
title: "Polymorphic Slot Composition"
capability_id: "SK-091"
domain: "04-radix-ui"
...
Design, implement, review and troubleshoot **Polymorphic Slot Composition** using `@radix-ui/react-slot` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Polymorphic Slot Composition**.
- A design or implementation decision depends on `@radix-ui/react-slot`.
...
- The task only asks whether `@radix-ui/react-slot` is installed.
...
1. Clarify the desired outcome for polymorphic slot composition and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-slot` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides polymorphic slot composition, not only a wrapper or installed dependency.
