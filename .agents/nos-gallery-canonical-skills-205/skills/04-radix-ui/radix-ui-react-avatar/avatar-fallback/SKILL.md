...
title: "Avatar and Fallback"
capability_id: "SK-066"
domain: "04-radix-ui"
...
Design, implement, review and troubleshoot **Avatar and Fallback** using `@radix-ui/react-avatar` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Avatar and Fallback**.
- A design or implementation decision depends on `@radix-ui/react-avatar`.
...
- The task only asks whether `@radix-ui/react-avatar` is installed.
...
1. Clarify the desired outcome for avatar and fallback and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-avatar` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides avatar and fallback, not only a wrapper or installed dependency.
