...
Design, implement, review and troubleshoot **Accessible Accordion** using `@radix-ui/react-accordion` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Accessible Accordion**.
- A design or implementation decision depends on `@radix-ui/react-accordion`.
...
- The task only asks whether `@radix-ui/react-accordion` is installed.
...
1. Clarify the desired outcome for accessible accordion and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-accordion` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides accessible accordion, not only a wrapper or installed dependency.
