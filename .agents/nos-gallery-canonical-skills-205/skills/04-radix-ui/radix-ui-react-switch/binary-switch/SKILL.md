...
Design, implement, review and troubleshoot **Binary Switch** using `@radix-ui/react-switch` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Binary Switch**.
- A design or implementation decision depends on `@radix-ui/react-switch`.
...
- The task only asks whether `@radix-ui/react-switch` is installed.
...
1. Clarify the desired outcome for binary switch and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-switch` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides binary switch, not only a wrapper or installed dependency.
