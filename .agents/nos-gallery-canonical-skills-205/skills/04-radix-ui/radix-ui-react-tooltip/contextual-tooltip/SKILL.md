...
Design, implement, review and troubleshoot **Contextual Tooltip** using `@radix-ui/react-tooltip` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Contextual Tooltip**.
- A design or implementation decision depends on `@radix-ui/react-tooltip`.
...
- The task only asks whether `@radix-ui/react-tooltip` is installed.
...
1. Clarify the desired outcome for contextual tooltip and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-tooltip` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides contextual tooltip, not only a wrapper or installed dependency.
