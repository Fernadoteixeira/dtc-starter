...
Design, implement, review and troubleshoot **Checkbox States** using `@radix-ui/react-checkbox` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Checkbox States**.
- A design or implementation decision depends on `@radix-ui/react-checkbox`.
...
- The task only asks whether `@radix-ui/react-checkbox` is installed.
...
1. Clarify the desired outcome for checkbox states and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-checkbox` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides checkbox states, not only a wrapper or installed dependency.
