...
Design, implement, review and troubleshoot **Exclusive Selection** using `@radix-ui/react-radio-group` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Exclusive Selection**.
- A design or implementation decision depends on `@radix-ui/react-radio-group`.
...
- The task only asks whether `@radix-ui/react-radio-group` is installed.
...
1. Clarify the desired outcome for exclusive selection and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-radio-group` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides exclusive selection, not only a wrapper or installed dependency.
