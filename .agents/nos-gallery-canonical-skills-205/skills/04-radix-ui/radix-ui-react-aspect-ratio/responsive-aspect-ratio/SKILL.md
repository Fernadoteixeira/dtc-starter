...
title: "Responsive Aspect Ratio"
capability_id: "SK-065"
domain: "04-radix-ui"
...
Design, implement, review and troubleshoot **Responsive Aspect Ratio** using `@radix-ui/react-aspect-ratio` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Responsive Aspect Ratio**.
- A design or implementation decision depends on `@radix-ui/react-aspect-ratio`.
...
- The task only asks whether `@radix-ui/react-aspect-ratio` is installed.
...
1. Clarify the desired outcome for responsive aspect ratio and identify the user-visible or operational result.
2. Inspect the relevant `@radix-ui/react-aspect-ratio` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides responsive aspect ratio, not only a wrapper or installed dependency.
