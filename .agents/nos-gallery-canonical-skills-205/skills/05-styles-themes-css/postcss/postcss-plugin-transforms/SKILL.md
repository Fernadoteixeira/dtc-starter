---
name: "postcss-plugin-transforms"
title: "PostCSS Plugin Transformations"
capability_id: "SK-116"
...
# PostCSS Plugin Transformations

## Purpose

Design, implement, review and troubleshoot **PostCSS Plugin Transformations** using `postcss` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **PostCSS Plugin Transformations**.
- A design or implementation decision depends on `postcss`.
...
- The task only asks whether `postcss` is installed.
...
1. Clarify the desired outcome for postcss plugin transformations and identify the user-visible or operational result.
2. Inspect the relevant `postcss` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides postcss plugin transformations, not only a wrapper or installed dependency.
