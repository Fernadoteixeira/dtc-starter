---
name: "enter-exit-states"
title: "Enter and Exit States"
capability_id: "SK-120"
...
Design, implement, review and troubleshoot **Enter and Exit States** using `tailwindcss-animate` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Enter and Exit States**.
- A design or implementation decision depends on `tailwindcss-animate`.
...
- The task only asks whether `tailwindcss-animate` is installed.
...
1. Clarify the desired outcome for enter and exit states and identify the user-visible or operational result.
2. Inspect the relevant `tailwindcss-animate` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides enter and exit states, not only a wrapper or installed dependency.
