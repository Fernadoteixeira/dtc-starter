---
name: "conditional-class-composition"
title: "Conditional Class Composition"
capability_id: "SK-108"
...
Design, implement, review and troubleshoot **Conditional Class Composition** using `clsx` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Conditional Class Composition**.
- A design or implementation decision depends on `clsx`.
...
- The task only asks whether `clsx` is installed.
...
1. Clarify the desired outcome for conditional class composition and identify the user-visible or operational result.
2. Inspect the relevant `clsx` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides conditional class composition, not only a wrapper or installed dependency.
