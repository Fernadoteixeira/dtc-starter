---
name: "tailwind-postcss-integration"
title: "Tailwind PostCSS Integration"
capability_id: "SK-117"
...
Design, implement, review and troubleshoot **Tailwind PostCSS Integration** using `@tailwindcss/postcss` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Tailwind PostCSS Integration**.
- A design or implementation decision depends on `@tailwindcss/postcss`.
...
- The task only asks whether `@tailwindcss/postcss` is installed.
...
1. Clarify the desired outcome for tailwind postcss integration and identify the user-visible or operational result.
2. Inspect the relevant `@tailwindcss/postcss` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides tailwind postcss integration, not only a wrapper or installed dependency.
