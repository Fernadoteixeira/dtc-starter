---
name: "dom-prop-filtering"
title: "DOM Prop Filtering"
capability_id: "SK-114"
...
Design, implement, review and troubleshoot **DOM Prop Filtering** using `@emotion/is-prop-valid` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **DOM Prop Filtering**.
- A design or implementation decision depends on `@emotion/is-prop-valid`.
...
- The task only asks whether `@emotion/is-prop-valid` is installed.
...
1. Clarify the desired outcome for dom prop filtering and identify the user-visible or operational result.
2. Inspect the relevant `@emotion/is-prop-valid` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides dom prop filtering, not only a wrapper or installed dependency.
