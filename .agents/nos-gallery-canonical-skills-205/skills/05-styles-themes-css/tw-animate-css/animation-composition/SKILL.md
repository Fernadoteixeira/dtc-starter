...
Design, implement, review and troubleshoot **Animation Composition** using `tw-animate-css` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Animation Composition**.
- A design or implementation decision depends on `tw-animate-css`.
...
- The task only asks whether `tw-animate-css` is installed.
...
1. Clarify the desired outcome for animation composition and identify the user-visible or operational result.
2. Inspect the relevant `tw-animate-css` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides animation composition, not only a wrapper or installed dependency.
