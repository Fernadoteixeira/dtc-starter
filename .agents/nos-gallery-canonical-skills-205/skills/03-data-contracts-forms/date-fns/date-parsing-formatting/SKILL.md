---
name: "date-parsing-formatting"
title: "Date Parsing and Formatting"
capability_id: "SK-052"
...
Design, implement, review and troubleshoot **Date Parsing and Formatting** using `date-fns` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Date Parsing and Formatting**.
- A design or implementation decision depends on `date-fns`.
...
- The task only asks whether `date-fns` is installed.
...
1. Clarify the desired outcome for date parsing and formatting and identify the user-visible or operational result.
2. Inspect the relevant `date-fns` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides date parsing and formatting, not only a wrapper or installed dependency.
