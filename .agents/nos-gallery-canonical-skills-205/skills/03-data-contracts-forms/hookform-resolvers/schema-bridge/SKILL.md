---
name: "schema-bridge"
title: "Schema Resolver Bridge"
capability_id: "SK-050"
...
# Schema Resolver Bridge

## Purpose

Design, implement, review and troubleshoot **Schema Resolver Bridge** using `@hookform/resolvers` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Schema Resolver Bridge**.
- A design or implementation decision depends on `@hookform/resolvers`.
...
- The task only asks whether `@hookform/resolvers` is installed.
...
1. Clarify the desired outcome for schema resolver bridge and identify the user-visible or operational result.
2. Inspect the relevant `@hookform/resolvers` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides schema resolver bridge, not only a wrapper or installed dependency.
