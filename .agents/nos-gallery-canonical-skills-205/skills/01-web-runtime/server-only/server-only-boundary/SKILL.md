...
title: "Server-only Boundary Enforcement"
capability_id: "SK-021"
...
Design, implement, review and troubleshoot **Server-only Boundary Enforcement** using `server-only` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Server-only Boundary Enforcement**.
- A design or implementation decision depends on `server-only`.
...
- The task only asks whether `server-only` is installed.
...
1. Clarify the desired outcome for server-only boundary enforcement and identify the user-visible or operational result.
2. Inspect the relevant `server-only` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides server-only boundary enforcement, not only a wrapper or installed dependency.
