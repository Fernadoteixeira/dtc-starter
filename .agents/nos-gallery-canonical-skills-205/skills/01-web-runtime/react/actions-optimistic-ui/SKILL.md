---
name: "actions-optimistic-ui"
title: "Actions and Optimistic UI"
capability_id: "SK-015"
...
Design, implement, review and troubleshoot **Actions and Optimistic UI** using `react` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Actions and Optimistic UI**.
- A design or implementation decision depends on `react`.
...
- The task only asks whether `react` is installed.
...
1. Clarify the desired outcome for actions and optimistic ui and identify the user-visible or operational result.
2. Inspect the relevant `react` contracts, runtime boundaries and surrounding architecture.
...
- [ ] The solution demonstrably provides actions and optimistic ui, not only a wrapper or installed dependency.
