...
title: "Loading, Error and Not Found Recovery"
capability_id: "SK-009"
domain: "01-web-runtime"
package: "next"
package_version: "16.2.11"
...
# Loading, Error and Not Found Recovery

## Purpose

Design, implement, review and troubleshoot **Loading, Error and Not Found Recovery** using `next` as a canonical capability, without assuming prior incorporation into the target repository.

## Use this skill when

- A requirement explicitly involves **Loading, Error and Not Found Recovery**.
- A design or implementation decision depends on `next`.
...
- The task only asks whether `next` is installed.
...
1. Clarify the desired outcome for loading, error and not found recovery and identify the user-visible or operational result.
2. Inspect the relevant `next` contracts, runtime boundaries and surrounding architecture.
...
- Preserve explicit server/client boundaries and minimize client-side JavaScript.
- Do not infer caching, rendering, or runtime behavior without tracing the route and data source.
- Define loading, empty, error, timeout and recovery states.

## Acceptance criteria

- [ ] The solution demonstrably provides loading, error and not found recovery, not only a wrapper or installed dependency.
