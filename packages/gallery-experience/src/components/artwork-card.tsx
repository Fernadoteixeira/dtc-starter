...
export function ArtworkCard({ artwork, isActive, onClick }: { artwork: any; isActive: boolean; onClick: () => void }) {
  return (
    <article
      onClick={onClick}
      className={`relative shrink-0 cursor-pointer transition-all duration-700 ease-[cubic-bezier(0.2,0.8,0.2,1)] ${isActive ? 'w-[35vw]' : 'w-[20vw] opacity-40'} aspect-[3/4] overflow-hidden`}
    >
      ...
      {isActive && artwork.scenes?.length > 0 && (
        <div className="absolute right-5 top-1/2 -translate-y-1/2 flex flex-col gap-2">
          {artwork.scenes.map((scene: any, idx: number) => (
            <div key={scene.id} className={`w-10 h-14 overflow-hidden border ${idx === 0 ? 'border-[#d6b08a]' : 'border-transparent'}`}>
