...
function SceneRail({ scenes, locale, onSceneClick, }) {
    if (!scenes || scenes.length === 0)
        return null;
    return (<div className="dtc-gallery__scene-rail" aria-label="Alternative views">
      {scenes.slice(0, 3).map((scene, index) => (<button key={scene.id} type="button" className="dtc-gallery__scene" onClick={(e) => {
                e.stopPropagation();
                if (onSceneClick)
                    onSceneClick(scene);
            }} title={translateGallery(locale, SCENE_LABEL_KEYS[index] ?? SCENE_LABEL_KEYS[0])}>
          <img src={scene.image.url} alt={scene.image.alt || scene.label} className="dtc-gallery__scene-image" width={scene.image.width} height={scene.image.height} loading="lazy"/>
        </button>))}
    </div>);
}
function InteractiveArtworkCard({ item, index, role, locale, onClick, onSceneClick, }) {
...
function NavigationControls({ current, total, onSelect, onPrev, onNext, }) {
    return (<nav className="dtc-gallery__navigation" aria-label="Gallery pagination">
      <div className="flex items-center gap-2">
        <button type="button" onClick={onPrev} aria-label="Previous artwork" className="dtc-gallery__nav-btn p-2 text-white/60 hover:text-white transition-colors">
          &larr;
        </button>

        <div className="flex items-center gap-1.5" role="tablist">
          {Array.from({ length: total }).map((_, index) => (<button key={index} type="button" role="tab" aria-selected={index === current} aria-label={`Go to artwork ${index + 1}`} onClick={() => onSelect(index)} className={`dtc-gallery__navigation-dot cursor-pointer transition-all ${index === current ? "dtc-gallery__navigation-dot--active w-6" : "w-2 opacity-50"}`}/>))}
        </div>

        <button type="button" onClick={onNext} aria-label="Next artwork" className="dtc-gallery__nav-btn p-2 text-white/60 hover:text-white transition-colors">
          &rarr;
        </button>
      </div>
    </nav>);
...
export function GalleryExperience({ items, collectionTitle = "Fio Vivo", collectionNumber = "01", collectionNarrative, initialItemHandle, locale: localeInput, reducedMotion, onItemView, onSceneView, onProductIntent, onShare, onProgressChange, }) {
    ...
    const formattedCounter = `${String(currentIndex + 1).padStart(2, "0")} / ${String(totalCount).padStart(2, "0")}`;
    ...
    return (<div data-gallery-experience="true" data-reduced-motion={reducedMotion ? "true" : undefined} className="dtc-gallery" lang={locale} role="region" aria-label="Fio Vivo Interactive Gallery">
      ...
      <div className="dtc-gallery__viewport" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onWheel={handleWheel}>
        <div className="dtc-gallery__track">
          ...
          {adjacentItem && totalCount > 1 && (<InteractiveArtworkCard item={adjacentItem} index={(currentIndex + 1) % totalCount} role="adjacent" locale={locale} onClick={goToNext}/>)}

          {continuationItem && totalCount > 2 && (<InteractiveArtworkCard item={continuationItem} index={(currentIndex + 2) % totalCount} role="continuation" locale={locale} onClick={() => selectIndex((currentIndex + 2) % totalCount)}/>)}
        </div>
      </div>
