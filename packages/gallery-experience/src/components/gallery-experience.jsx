...
export function GalleryExperience({ items }) {
    ...
    return (<div className="gallery-shell relative h-full w-full overflow-hidden text-[#e6e2dd] bg-[#1a1918]" style={{ height: "calc(100svh - 64px)" }}>
      ...
      <div className="gallery-viewport relative z-10 flex h-full w-full items-center pl-[35vw]">
        <div className="flex gap-[4vw] items-center transition-transform duration-700 ease-[cubic-bezier(0.2,0.8,0.2,1)]" style={{ transform: `translateX(calc(-${currentIndex} * (35vw + 4vw)))` }}>
          {fixture.map((artwork, idx) => (<ArtworkCard key={artwork.id} artwork={artwork} isActive={idx === currentIndex} onClick={() => setCurrentIndex(idx)}/>))}
        </div>
      </div>

      {/* Navegação Inferior */}
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 flex gap-4">
        {fixture.map((_, i) => (<button key={i} onClick={() => setCurrentIndex(i)} className={`w-2 h-2 rounded-full transition-colors ${i === currentIndex ? 'bg-[#d6b08a]' : 'bg-[#d6b08a]/30'}`}/>))}
      </div>
