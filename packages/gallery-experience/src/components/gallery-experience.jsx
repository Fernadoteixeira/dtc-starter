...
function StaticSceneRail({ scenes }) {
    return (<div className="dtc-gallery__scene-rail" aria-hidden="true">
      {scenes.slice(0, 3).map((scene) => (<div key={scene.id} className="dtc-gallery__scene">
          <img src={scene.image.url} alt="" className="dtc-gallery__scene-image" width={scene.image.width} height={scene.image.height}/>
        </div>))}
    </div>);
}
function StaticArtworkCard({ item, role, }) {
...
export function GalleryExperience({ items, collectionTitle = "Fio Vivo", collectionNumber = "01", collectionNarrative = "O crochê se move", }) {
    ...
    return (<div data-gallery-experience="true" className="dtc-gallery">
      ...
      <div className="dtc-gallery__viewport">
        <div className="dtc-gallery__track">
          ...
          {adjacentItem && (<StaticArtworkCard item={adjacentItem} role="adjacent"/>)}
          {continuationItem && (<StaticArtworkCard item={continuationItem} role="continuation"/>)}
        </div>
      </div>
