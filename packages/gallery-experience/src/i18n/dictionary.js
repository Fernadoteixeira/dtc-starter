export const DEFAULT_GALLERY_LOCALE = "pt-BR";
 ...
 */
export function resolveGalleryLocale(input) {
    if (!input)
        return DEFAULT_GALLERY_LOCALE;
    const normalized = input.trim().toLowerCase();
    return LOCALE_ALIASES[normalized] ?? DEFAULT_GALLERY_LOCALE;
}
export const galleryDictionaries = {
    "pt-BR": {
 ...
 */
export function translateGallery(locale, key, params) {
    const template = galleryDictionaries[locale]?.[key] ??
        galleryDictionaries[DEFAULT_GALLERY_LOCALE]?.[key] ??
        key;
    if (!params)
        return template;
    return Object.entries(params).reduce((acc, [paramKey, value]) => acc.replaceAll(`{${paramKey}}`, String(value)), template);
}
/**
 * Product title translations. "a informar" handles are intentionally not
 * localized (untranslated placeholders stay as-is).
 */
export const fioVivoTitleTranslations = {
...
export function translateFioVivoTitle(handle, fallbackTitle, locale) {
